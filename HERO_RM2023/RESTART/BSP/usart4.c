...
void UART4_IRQHandler(void)
{
    ...
    {
				(void)UART4->SR;
				(void)UART4->DR;//进中断后读SR，再读DR寄存器后清除本次中断USART_IT_IDLE标志位，否则读到的值落后了一帧
				DMA_Cmd(DMA1_Stream2, DISABLE);//关闭接收DMA
				DMA_ClearFlag(DMA1_Stream2, DMA_FLAG_TCIF2 | DMA_FLAG_HTIF2);//清除标志位
				length = UART4_RX_BUF_LENGTH - DMA_GetCurrDataCounter(DMA1_Stream2);//获得接收帧长度 
				//函数DMA_GetCurrDataCounter(DMA2_Stream5); 获取当前指针计数值,用内存缓冲区大小 - 此计数值 = 接收到的数据长度
				targetOffsetDataDeal(length, UART4_DMA_RX_BUF );//接收数据处理
				DMA_SetCurrDataCounter(DMA1_Stream2,UART4_RX_BUF_LENGTH);//设置传输数据长度
