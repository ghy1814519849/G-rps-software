...
typedef enum
{
  ...
  GAME_ROBOT_HP_ID                   =0x0003,//机器人存活数据：0x0003。发送频率：1Hz
	DART_STATE_ID                      =0x0004,//飞镖发射状态数据：0x0004，比赛结束后发送
  ...
  BULLET_REMAINING_ID                =0x0208,//子弹剩余发射数：0x0208。发送频率：1Hz 周期发送，空中机器人以及哨兵机器人主控发送
	RFID_STATES_ID                     =0x0209,//机器人 RFID 状态:0x0209。发射速率:1Hz,发射范围:单一机器人
...
/**
  * @brief  2021串口接口协议说明
  */
typedef __packed struct // 比赛状态数据：0x0001。发送频率：1Hz
{
  uint8_t game_type : 4;
...
typedef __packed struct //比赛机器人状态：0x0201。发送频率：10Hz
{
  uint8_t robot_id;
  uint8_t robot_level;
  uint16_t remain_HP;
...
typedef __packed struct //子弹剩余发射数：0x0208。发送频率：1Hz 周期发送，空中机器人以及哨兵机器人主控发送 超级对抗赛中对英雄无用
{
...
typedef __packed struct //机器人 RFID 状态:0x0209。发射速率:1Hz,发射范围:单一机器人
{
...
extern uint8_t  tx_buf[BSP_UART5_DMA_TX_BUF_LEN];
extern uint8_t  pdata[19];
extern uint8_t  ddata[62];
extern float pitch_data;
extern float yaw_data;

extern int hero_42mm_shot_bullet_num;
extern int hero_remain_last_hp;
extern int hero_remain_hp;
extern uint32_t event_X2_data;
extern uint32_t event_X3_data;
extern uint32_t event_X4_data;
...
extern uint8_t _UART5_DMA_RX_BUF[2][BSP_UART5_DMA_RX_BUF_LEN];
